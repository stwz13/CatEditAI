...
int BinaryOp_x86_avx::forward_bf16s(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    const Mat& A = bottom_blobs[0];
    const Mat& B = bottom_blobs[1];
    const int outdims = std::max(A.dims, B.dims);
...
int BinaryOp_x86_avx::forward_inplace_bf16s(Mat& bottom_top_blob, const Option& opt) const
{
