...
int Reshape_x86_fma::forward_bf16s_fp16s(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
#if NCNN_BATCH
    if (input_batch_axis != 233 || output_batch_axis != 233)
...
#if NCNN_BATCH
int Reshape_x86_fma::forward_batch(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& opt) const
{
    const Mat& bottom_blob = bottom_blobs[0];
    Mat& top_blob = top_blobs[0];
